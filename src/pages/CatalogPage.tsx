...
const CATEGORY_LABELS: Record<ProductCategory, string> = { 'action-figures': 'Figuras de Acción', 'video-games': 'Videojuegos', shoes: 'Zapatillas' };
const STOREFRONT_LIMIT = 100;
...
export function CatalogPage() {
  ...
  const { products, isLoading, error } = useProducts({ searchTerm: localSearch, category: categoryParam as ProductCategory | 'all', limit: STOREFRONT_LIMIT });
  ...
  const handleAddToCart = (product: Product) => { addItem(product, 1); setQuickViewProduct(null); };

  return <>
    <Header />
    <Container className="py-8 sm:py-12">
      <div className="mb-8 flex items-center gap-2 text-sm"><Link to={ROUTES.HOME} className="text-slate-500 transition hover:text-cyan-300">Inicio</Link><span className="text-slate-700">/</span><span className="text-cyan-200">Catálogo</span></div>
      <div className="mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="cyber-kicker">PRODUCT DATABASE // ONLINE</p><h1 className="mt-2 font-display text-3xl font-black uppercase text-white sm:text-4xl">Catálogo</h1><p className="mt-2 text-slate-400">Encuentra tu próximo videojuego, coleccionable o par de zapatillas.</p></div><div className="w-full max-w-xl"><ProductSearch value={localSearch} onChange={setLocalSearch} placeholder="Buscar productos..." /></div></div>
