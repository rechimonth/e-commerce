...
export const productsService = {
  async fetchProducts(params: FetchProductsParams = {}): Promise<ProductsResult> {
    return firebaseTryCatch(async () => { const { search, category, limit = DEFAULT_LIMIT } = params; const activeProducts = await getProducts({ category, isActive: true, limit }); const allItems = activeProducts.map(toProduct); const filtered = search ? allItems.filter((p) => p.name.toLowerCase().includes(search.toLowerCase())) : allItems; return { items: filtered, pagination: { page: 1, limit, total: filtered.length, hasNext: filtered.length >= limit, hasPrev: false } }; });
  },
  ...
  async deactivateProduct(id: string): Promise<Product | null> { return firebaseTryCatch(async () => { const dto = await firestoreUpdateProduct(id, { isActive: true }); return dto ? toProduct(dto) : null; }); },
};
