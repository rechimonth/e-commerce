...
function setCorsHeaders(req: VercelRequest, res: VercelResponse) {
  ...
  if (origin && (configured.includes(origin) || origin === vercelOrigin)) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      for (const [productId, ref] of productRefs) {
        productSnapshots.set(productId, await transaction.get(ref));
      }
      ...
      for (const item of body.items) {
        if (orderItems.some((orderItem) => orderItem.productId === item.productId)) continue;
        const snapshot = productSnapshots.get(item.productId);
        ...
        if (
          ...
          throw new Error(`PRODUCT_INVALID:${item.productId}`);
        }
        ...
        if (!Number.isSafeInteger(lineTotal) || !Number.isSafeInteger(subtotalCents + lineTotal)) {
          throw new Error('ORDER_TOTAL_TOO_LARGE');
        }
